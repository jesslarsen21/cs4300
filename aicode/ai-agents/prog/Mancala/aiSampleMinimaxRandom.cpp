...
namespace ai
{
  namespace Sample
  {
    MinimaxRandom::MinimaxRandom(ai::Agent::Options *opts)
      : max_depth(3)
    {
      ...
      {
        char buf[128];
        std::sprintf(buf, "MinimaxRandom-%d", this->max_depth);
        SetName(buf);
      }
    ...
    MinimaxRandom::~MinimaxRandom()
    {
    }
    
    ai::Agent::Action * MinimaxRandom::Program(const ai::Agent::Percept * percept)
    {
    ...
    PlyData MinimaxRandom::PickMove(const ai::Mancala::Board &board, int player)
    {
      PlyData ply_data = this->Max(board, player, 1);
      return ply_data;
    }

    PlyData MinimaxRandom::Max(const ai::Mancala::Board &board, int player, int depth)
    {
    ...
    PlyData MinimaxRandom::Min(const ai::Mancala::Board &board, int player, int depth)
    {
      PlyData ply_data(1.e6);
      if(board.HaveWinner() || depth > this->max_depth)
        {
     ...
     */
    double MinimaxRandom::Evaluate(const ai::Mancala::Board &board, int player)
    {
      int other_player = (player == 1) ? 2 : 1;
